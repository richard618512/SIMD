...
//本人将对代码的一些理解写到了注释里，绝对都是本人自己的思考，就是可能有点啰嗦见谅......
	...
	//先解释一下串行代码：for (int i1 = 0; i1 < 16; ++i1)
		...
		for (int block = 0; block < min_blocks; block++) {//用最小数量块避免越界访问
        // 加载4个消息的当前块到NEON寄存器
        uint32x4_t X[16][4]; // X[i][j]表示第j个信息的第i个32位字
        
        for (int msg = 0; msg < 4; msg++) {
            const Byte* block_start = paddedMessages[msg] + block * 64;//定位到第msg条信息的第block块的起始位置，准备下一次并行处理
            for (int i = 0; i < 16; i++) {
	 ...
	 for (int msg = 0; msg < 4; msg++) {
		...
		// 提取并转换 state 的字节序
		for (int i = 0; i < 4; i++) {
			uint32_t value = vgetq_lane_u32(states_neon[i], msg);
			state[i] = __builtin_bswap32(value); // 使用编译器内置函数优化
		}
		
		// 存储到最终输出
		memcpy(&states[msg], state, sizeof(state));
	}
	 //这里就是将state的字节序转换为小端序，进行返回
	 // 释放内存
	 for (int i = 0; i < 4; i++) {
